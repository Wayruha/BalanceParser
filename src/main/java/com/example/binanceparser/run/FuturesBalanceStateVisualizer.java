...
public class FuturesBalanceStateVisualizer {
	...
	public static void main(String[] args) throws IOException {
		...
		final List<BalanceReport> reports = visualizer.futuresBalanceVisualisation();
		reports.forEach(r -> System.out.println(r.toPrettyString() + "\n"));
	}

	public List<BalanceReport> futuresBalanceVisualisation() throws IOException {
		final DataWriter<BalanceReport> reportWriter = Helper.getReportWriter(appProperties.getReportOutputType(), config);
		final var reportSerializerPostProcessor = new ReportSerializer(reportWriter);

		final var processor = new MultipleUsersFuturesBalProcessor(eventSource, config, nameSource);
		processor.registerPostProcessor(reportSerializerPostProcessor);
		final List<BalanceReport> testReport = processor.process();
		return testReport;
	}
