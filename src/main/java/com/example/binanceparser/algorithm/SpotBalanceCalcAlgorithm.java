...
public class SpotBalanceCalcAlgorithm implements CalculationAlgorithm {
    final int MAX_ALLOWED_DELAY_BTW_RELATED_EVENTS = 1;
    ...
    public List<BalanceState> processEvents(List<AbstractEvent> events, List<String> assetsToTrack) {
        ...
        for (int i = 0; i < events.size() - 1; i++) {
            ...
            if (ChronoUnit.SECONDS.between(firstEvent.getDate(), nextEvent.getDate()) > MAX_ALLOWED_DELAY_BTW_RELATED_EVENTS)
                continue;
            ...
            if (orderEvent.getSide().equals("BUY") && assetRate.containsKey(orderSymbol)) {
                ...
                BigDecimal sum = newQuantity.multiply(newPrice)
                        .add(currentQuantity.multiply(currentPrice));
            ...
            BalanceState balanceState = new BalanceState(accEvent.getDate().toLocalDate(), new HashSet<>(actualBalance.values()));
