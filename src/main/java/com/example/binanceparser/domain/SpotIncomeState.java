...
public class SpotIncomeState extends BalanceState {
    ...
    public void updateAssetBalance(List<Asset> updatedAssets) {
    	...
    	processOrderDetails(USDT, addAssetIfNotExist(USDT).getAvailableBalance(), BigDecimal.ONE);
    	processOrderDetails(BUSD, addAssetIfNotExist(BUSD).getAvailableBalance(), BigDecimal.ONE);
    }
    ...
    public void processOrderDetails(String assetName, BigDecimal assetDelta, BigDecimal transactionPrice) {
		...
		if (assetDelta.compareTo(BigDecimal.ZERO) < 0) {
			if (transactionPrice != null) {
				...
				//WITHDRAW operation
				processOrderDetails(assetName, assetState.getAvailableBalance().compareTo(assetDelta) >= 0 ? assetDelta
