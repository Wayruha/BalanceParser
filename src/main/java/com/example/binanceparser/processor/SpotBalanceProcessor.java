...
public class SpotBalanceProcessor extends Processor<BalanceVisualizerConfig> {
    ...
    final TestSpotBalancecalcAlgorithm algorithm;
    ...
    public BalanceReport process() throws IOException {
        ...
        List<SpotIncomeState> balanceStates = algorithm
        		.processEvents(events)
        		.stream()
        		.filter(state -> inRange(state, config.getStartTrackDate(), config.getFinishTrackDate()))
        		.collect(Collectors.toList());
        
        final BalanceReport balanceReport = balanceReportGenerator.getStateReport(balanceStates);

        System.out.println("Processor done for config: " + config);
        return balanceReport;
    }

    private boolean inRange(SpotIncomeState state, LocalDateTime rangeStart, LocalDateTime rangeEnd){
